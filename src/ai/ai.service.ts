...
import { LlamaAIAdapter, MistralAIAdapter, OpenAIAdapter } from './api-providers/index.ts';
import type { AIProvidable } from './api-providers/index.ts';
import { ClaudeAIAdapter } from './api-providers/ClaudeAIAdapter.ts';
...
export const pickAIProvider = (): AIProvidable => {
  const providerFactories = [
    ...
    {
      enabled: config.llamaApiEnabled,
      apiKey: config.llamaApiKey,
      model: config.llamaModel,
      create: () =>
        new LlamaAIAdapter(
          config.llamaApiKey,
          pickModelId(config.llamaModel),
          config.llamaMaxCompletionTokens,
          config.llamaTimeoutMs
        ),
    },
  ];
